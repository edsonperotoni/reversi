...
int avalia_tabuleiro(JOGADOR pl, char computer)
{
	...
	for (row = 0; row <= 7; row++)
		for (col = 0; col <= 7; col++)
		{
			...
			else if (Tabuleiro_atual[row][col] == _other)
				val -= VlrHeur[row][col];
		}
...
Lista_Mov *ordena(Lista_Mov *inicio)
{
	...
	{
		if ((mov_possiveis->row == 0 && mov_possiveis->col == 7) || (mov_possiveis->row == 7 && mov_possiveis->col == 0) || (mov_possiveis->row == 7 && mov_possiveis->col == 7))
		{
...
{
	if (row >= 0 && row <= 7 && col >= 0 && col <= 7 && Tabuleiro_atual[row][col] == Vazio && existe_trocas(row, col, pl))
		return (1);
	return (0);
}
...
void inicializa_tabuleiro(void)
{
	...
	Tabuleiro_anterior[3][3] = 'X';
	Tabuleiro_anterior[3][4] = 'O';
	Tabuleiro_anterior[4][3] = 'O';
	Tabuleiro_anterior[4][4] = 'X';

	Tabuleiro_atual[3][3] = 'X';
	Tabuleiro_atual[3][4] = 'O';
	Tabuleiro_atual[4][3] = 'O';
	Tabuleiro_atual[4][4] = 'X';
...
void mostra_pecas(char tomove, char jogou_antes, char computer, int posicao_atual, int posicao_anterior)
{
	...
	gotoxy(7, 2);
	textcolor(WHITE);
	cprintf("%s", "Jogada atual:");
	gotoxy(20, 2);
	textcolor(BROWN);
	cprintf("%c", tomove);
	gotoxy(22, 2);
	textcolor(WHITE);
	cprintf("%s", tomove == computer ? computador : humano);
	gotoxy(33, 2);
	textcolor(BROWN);
	...
	gotoxy(45, 2);
	textcolor(WHITE);
	cprintf("%s", "Jogada anterior:");
	gotoxy(61, 2);
	textcolor(BROWN);
	cprintf("%c", jogou_antes);
	gotoxy(63, 2);
	textcolor(WHITE);
	cprintf("%s", jogou_antes == computer ? computador : humano);
	...
	for (i = 0; i <= 7; i++)
	{
		...
		for (j = 0; j <= 7; j++)
		{
			if (8 * i + j == posicao_anterior)
				textcolor(BROWN);
			else if (Tabuleiro_anterior[i][j] == 'X')
				textcolor(LIGHTBLUE);
			else
				textcolor(RED);
...
void mostra_tabuleiro(char tomove, char computer)
{
	...
	else if (tomove == computer && computer == 'O')
	{
		textcolor(BROWN);
		cprintf("%c", 'X');
	}
	else if (tomove != computer && computer == 'X')
	{
		textcolor(BROWN);
		cprintf("%c", 'X');
	}
	...
	gotoxy(67, 2);
	textcolor(WHITE);
	cprintf("%s", tomove == computer ? humano : computador);
...
void main(void)
{
	...
	do
	{
		...
		gotoxy(19, 8);
		textcolor(LIGHTBLUE);
		cprintf("3 - tem que pensar...");
		gotoxy(19, 9);
		textcolor(LIGHTBLUE);
		cprintf("4 - complicou!");
		gotoxy(19, 10);
		textcolor(LIGHTBLUE);
		cprintf("5 - experiente");
		gotoxy(19, 11);
		textcolor(LIGHTBLUE);
		cprintf("6 - mestre!");
				...
				else
				{
					...
					if (depth > 0)
					{
						...
						minimax(computer, computer == 'X' ? JogadorUm : JogadorDois,
										3 + depth, &posicao, -Infinito, Infinito);
					}
					else
					{
						int i = 0, n;
